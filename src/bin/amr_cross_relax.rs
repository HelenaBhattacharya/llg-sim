...
fn main() {
    ...
    // ---- Time stepping ----
    // α = 0.5 gives strong relaxation damping (standard for relaxation benchmarks).
    // dt = 2e-13 is ~4× larger — stable for this exchange-dominated soft magnet at
    // these cell sizes, and means each step covers more physical evolution.
    // 2000 steps is plenty to see the cross pattern collapse into the vortex ground state.
    let alpha: f64 = env_or("LLG_CROSS_ALPHA", 0.5);
    let dt: f64 = env_or("LLG_CROSS_DT", 2.0e-13);
    let steps_base: usize = env_or("LLG_CROSS_STEPS", 2000);
    let out_every_base: usize = env_or("LLG_CROSS_OUT_EVERY", 100);
    ...
    println!();
    println!("╔{}╗", bar);
    println!("║{:^64}║", "AMR CROSS RELAXATION — RESULTS SUMMARY");
    println!("╚{}╝", bar);
