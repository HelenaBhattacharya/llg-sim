...
fn main() {
    ...
    // Compute derived values for the summary
    let l1_final = h.patches.len();
    let l2_final = h.patches_l2plus.get(0).map(|v| v.len()).unwrap_or(0);
    let l3_final = h.patches_l2plus.get(1).map(|v| v.len()).unwrap_or(0);

    let final_n = h.num_levels();
    let final_ratio = if subcycle_active {
        pow_usize(refine_ratio * refine_ratio, final_n.saturating_sub(1))
    } else {
        1
    };
    let amr_coarse_steps = if final_ratio > 0 { steps / final_ratio.max(1) } else { steps };

    let avg_amr_ms = if amr_step_count > 0 {
        (t_amr_step / amr_step_count as f64) * 1e3
    } else {
        f64::NAN
    };
    let avg_fine_ms = if !skip_fine_ref && steps > 0 {
        (t_demag_fine / steps as f64) * 1e3
    } else {
        f64::NAN
    };
    let speedup_per_step = if avg_amr_ms > 0.0 && avg_fine_ms.is_finite() {
        avg_fine_ms / avg_amr_ms
    } else {
        f64::NAN
    };

    // Wall-time speedup: total time running uniform fine vs total AMR time
    let wall_fine = t_demag_fine; // total time spent on fine reference
    let wall_amr = t_amr_step;   // total time spent on AMR steps
    let speedup_wall = if wall_amr > 0.0 && !skip_fine_ref {
        wall_fine / wall_amr
    } else {
        f64::NAN
    };
    ...
    println!("  {thin}");
    println!("  Level 1 patches    {}", l1_final);
    println!("  Level 2 patches    {}", l2_final);
    println!("  Level 3 patches    {}", l3_final);
    println!("  Fine cells total   {:>12}   ({} × {})", fine_cells_total, fine_nx, fine_ny);
    println!("  Fine cells in AMR  {:>12}   ({:.1}% coverage)", fine_cells_in_patches, coverage * 100.0);
    println!("  Cell savings       {:>12}   ({:.1}% fewer cells)",
        fine_cells_total - fine_cells_in_patches,
        (1.0 - coverage) * 100.0);
    ...
    if speedup_wall.is_finite() {
        println!("  Wall time: {:.1} s  (fine)  vs  {:.1} s  (AMR)  →  {:.1}× faster",
            wall_fine, wall_amr, speedup_wall);
    }
    ...
    println!("  {thin}");
    println!("  Uniform fine FFT   {:>6} × {:<6} = {:>10} cells", fine_nx, fine_ny, fine_nx * fine_ny);
    println!("  Coarse FFT         {:>6} × {:<6} = {:>10} cells", base_nx, base_ny, base_nx * base_ny);
